...
- Access to the target database host
- A backup file generated by this project (`.sql.gz`)
...
```bash
zcat path/to/backup/dump_file.sql.gz | mysql -h 127.0.0.1 -P 3306 -u your_user -p your_database
```

### Restore to a remote database
```bash
zcat path/to/backup/dump_file.sql.gz | mysql -h your-db-host -P 3306 -u your_user -p your_database
```

### Restore to an uncompressed dump (.sql)
```bash
mysql -h your-db-host -P 3306 -u your_user -p your_database < path/to/backup/dump_file.sql
```

### Verify the restoration
Check if tables are present:
```bash
mysql -h your-db-host -P 3306 -u your_user -p -e "USE your_database; SHOW TABLES;"
```

### Common issues
- `Access denied`: verify user, password, and host permissions.
- `Unknown database`: create the database first or use the correct database name.
